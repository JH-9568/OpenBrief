...
- token 값을 채팅창에 붙여넣지 않습니다. 에이전트가 화면을 제어하더라도 token 문자열은 사용자가 직접 복사하고 숨김 입력에 직접 붙여넣습니다.
- OpenBrief는 기본적으로 원본 Figma, Notion, Discord, GitHub를 수정하지 않고 읽기/정리만 합니다.
- `pip install --user`로 설치한 경우 `openbrief`가 PATH에 없을 수 있습니다. 이때는 `/Users/<사용자명>/.local/bin/openbrief`처럼 전체 경로로 실행하거나 PATH 설정을 안내받습니다.
- 기본 포트 `8000`이 이미 사용 중이면 OpenBrief가 다른 포트로 실행될 수 있습니다. `openbrief status`로 실제 대시보드 주소를 확인합니다.
...
- 가능하면 pipx로 설치해줘.
- pipx가 없으면 설치 방법을 먼저 안내하고, 임시 대안으로 `python -m pip install --user "git+https://github.com/JH-9568/OpenBrief.git"`를 제시해줘.
- 설치 명령은 `pipx install "git+https://github.com/JH-9568/OpenBrief.git"`를 사용해줘.
- 설치 후 `openbrief --help`를 실행해서 정상 설치 여부를 확인해줘.
- 만약 `openbrief` 명령을 찾지 못하면 `python -m site --user-base`로 user base를 확인하고, macOS/Linux에서는 `<user-base>/bin/openbrief` 전체 경로로 다시 확인해줘.
- 그 다음 `openbrief init`을 실행해줘.
- `openbrief start --daemon --no-browser`로 실행하고 `openbrief status`로 실제 접속 주소를 확인해줘.
- OpenBrief는 내 컴퓨터에서만 실행되는 로컬 웹앱이고 기본 대시보드는 http://127.0.0.1:8000/dashboard 라는 점을 설명해줘.
- 만약 8000 포트가 이미 사용 중이면 OpenBrief가 8010 같은 다른 포트로 뜰 수 있으니 `openbrief status`에 나온 주소를 기준으로 열어줘.
- API key나 token이 필요한 단계에서는 내가 직접 입력하게 해줘.
- token을 명령어 인자로 넣지 말고 `openbrief auth ...` 방식의 숨김 입력을 사용해줘.
```

### macOS `.app` launcher 생성 프롬프트

```text
내 Mac에서 OpenBrief를 더블클릭으로 실행할 수 있게 `.app` launcher까지 만들어줘.

조건:
- 먼저 OpenBrief CLI가 설치되어 있는지 `openbrief --help`로 확인해줘.
- `openbrief`가 PATH에 없으면 `/Users/<사용자명>/.local/bin/openbrief` 같은 전체 경로도 확인해줘.
- 공식 저장소를 clone하거나 이미 clone된 폴더가 있으면 그 폴더를 사용해줘.
- 저장소 루트에서 `scripts/build_macos_app.sh`를 실행해줘.
- 생성 결과 `dist/OpenBrief.app`이 있는지 확인해줘.
- 원하면 `cp -R dist/OpenBrief.app /Applications/OpenBrief.app`로 Applications에 복사해서 설치해줘.
- `open /Applications/OpenBrief.app` 또는 `open dist/OpenBrief.app`로 실행 테스트해줘.
- `.app`은 OpenBrief 본체를 포함한 완전한 설치 파일이 아니라, 설치된 `openbrief start --daemon`을 실행하고 기본 브라우저로 로컬 대시보드를 여는 launcher라는 점을 설명해줘.
- 현재 버전은 별도 네이티브 앱 창/WebView로 뜨지 않고 브라우저로 열린다는 점을 설명해줘.
- 기본 대시보드는 http://127.0.0.1:8000/dashboard 이지만, 8000 포트가 사용 중이면 `openbrief status`에 나온 실제 포트를 사용해야 한다고 설명해줘.
- 현재 `.app`은 서명/공증되지 않으므로 macOS Gatekeeper 경고가 뜰 수 있다는 점을 설명해줘.
- `.app` 생성과 GitHub Release 배포는 무료이고, Apple Developer Program을 통한 서명/공증은 선택 사항이며 유료라는 점을 설명해줘.
```
...
- `openbrief start --daemon --no-browser` 실행
- `openbrief status`로 실제 대시보드 주소를 확인
- 브라우저에서 대시보드 주소를 열어서 프로젝트와 브리프가 보이는지 확인
- 공개 GitHub 저장소는 token 없이도 테스트 가능하다는 점을 설명
- 대시보드에서 evidence가 일부만 표시될 수 있으니 sync 결과의 fetched/stored 개수와 화면 표시 개수를 구분해서 설명
- 문제가 생기면 로그 위치와 해결 방법을 알려줘.
```

### Figma 파일 하나만 연결하는 프롬프트

```text
OpenBrief에 Figma 파일 하나만 먼저 연결해서 테스트해줘.

조건:
- 내가 Figma file URL을 줄게.
- `openbrief setup --project "<프로젝트명>" --figma-file-url "<Figma URL>"`로 프로젝트를 등록해줘.
- token은 명령어 인자로 넣지 말고 `openbrief auth figma --project-id <프로젝트ID>`로 숨김 입력하게 해줘.
- Figma token이 없으면 발급 위치를 알려줘:
  - Figma 우측 상단 프로필
  - Settings
  - Security
  - Personal access tokens
  - Generate new token
- 필요한 Figma 권한은 최소 읽기 권한으로 안내해줘:
  - file_content:read
  - file_metadata:read
  - file_comments:read
- token 문자열은 채팅에 붙여넣지 말라고 알려줘. 내가 Figma 화면에서 직접 복사하고 터미널 숨김 입력에 붙여넣게 해줘.
- token 저장 후 `openbrief sync --provider figma --project-id <프로젝트ID> --brief`를 실행해줘.
- 성공하면 fetched/stored 개수, 생성된 brief revision, 대시보드 URL을 알려줘.
- 원본 Figma 파일은 수정하지 않고 읽기만 한다는 점을 확인해줘.
```
...
- token/API key를 채팅에 붙여넣으라고 하지 마.
- `--token`, `--openai-api-key`, `--github-token` 같은 인자 방식은 피하고 `openbrief auth ...`를 사용해.
- 각 서비스에서 필요한 값이 무엇인지 먼저 알려줘.
- 필요한 값은 GitHub `owner/repo`, Figma file URL, Notion page URL, Discord channel id야.
- 내가 값을 준비하면 한 번에 다 연결하지 말고 하나씩 연결하고 `openbrief sync --brief`로 테스트해줘.
- 각 연결마다 성공 여부, 수집 개수, 실패 시 권한/접근 문제를 분리해서 설명해줘.
...
openbrief start --daemon --no-browser
openbrief status
```
...
8000 포트가 이미 사용 중이면 `openbrief status`에 표시된 실제 주소를 사용합니다. 예를 들어 `http://127.0.0.1:8010/dashboard`처럼 다른 포트로 뜰 수 있습니다.
...
현재 `.app`은 네이티브 앱 창이나 WebView 앱이 아닙니다. 더블클릭하면 설치된 `openbrief`를 실행하고 기본 브라우저에서 로컬 대시보드를 엽니다. “앱 창 안에서 열리는” 형태는 이후 WebView 패키징 단계에서 추가할 수 있습니다.
...
cp -R dist/OpenBrief.app /Applications/OpenBrief.app
open /Applications/OpenBrief.app
```

이 launcher는 새 앱 번들이지만 OpenBrief 본체를 포함하지 않습니다. 내부적으로 설치된 `openbrief start --daemon`을 실행하고 브라우저로 로컬 대시보드를 엽니다. 기본 주소는 `http://127.0.0.1:8000/dashboard`이며, 8000 포트가 사용 중이면 다른 포트로 실행될 수 있습니다.
...
토큰을 명령어 인자로 넣으면 shell history에 남을 수 있습니다. 실사용에서는 `openbrief auth`로 숨김 입력합니다.
...
프로젝트가 여러 개라면 provider token을 특정 프로젝트에 저장합니다.

```bash
openbrief auth figma --project-id "<project-id>"
openbrief auth notion --project-id "<project-id>"
openbrief auth discord --project-id "<project-id>"
openbrief auth github --project-id "<project-id>"
```
...
`--token`, `--openai-api-key`, `--github-token` 같은 인자 입력은 shell history에 남을 수 있으므로 사용하지 않는 것을 권장합니다. token/API key는 채팅창에도 붙여넣지 말고 `openbrief auth ...`의 숨김 입력에 직접 붙여넣습니다.
...
Endpoint와 model은 대시보드의 AI 설정 화면에서 수정할 수 있습니다.

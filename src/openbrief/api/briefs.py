...
from openbrief.briefs.service import (
    approval_state,
    approve_revision,
...
from openbrief.models import BriefRevision, Project
from openbrief.notifications.discord import send_discord_brief_notification
from openbrief.schemas import (
    ApprovalRead,
    BriefEditRequest,
...
async def approve_brief(
    project_id: uuid.UUID,
    revision_id: uuid.UUID,
    x_openbrief_member_id: uuid.UUID = Header(alias="X-OpenBrief-Member-ID"),
    session: AsyncSession = Depends(get_session),
) -> ApprovalRead:
    revision = await session.get(BriefRevision, revision_id)
    if revision is None or revision.project_id != project_id:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Brief revision not found")
    try:
        return await approve_revision(session, revision_id, x_openbrief_member_id)

...
from openbrief.scheduler import run_daily_project_briefs
from openbrief.sources.service import list_source_items

settings = get_settings()
celery_app = Celery("openbrief", broker=settings.redis_url, backend=settings.redis_url)
celery_app.conf.update(
    ...
    beat_schedule={
        "run-daily-project-briefs": {
            "task": "openbrief.run_daily_project_briefs",
            "schedule": crontab(
...
@celery_app.task(name="openbrief.generate_daily_brief")
...
@celery_app.task(name="openbrief.generate_daily_brief_and_notify")
...
@celery_app.task(name="openbrief.poll_discord_channel")
...
@celery_app.task(name="openbrief.run_daily_project_briefs")

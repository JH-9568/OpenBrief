...
async def test_edit_brief_supersedes_revision_and_preserves_sources(session):
    workspace = Workspace(name="Acme")
    session.add(workspace)
    await session.flush()
    project = Project(workspace_id=workspace.id, name="Launch")
    ...
    content["diff_from_last_confirmed"] = ["Edited wording"]
    app = create_app()
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        response = await client.post(
            f"/api/v1/projects/{project.id}/briefs/{revision.id}/edit",
            json={"content": content, "created_by": "tester"},
        )

    assert response.status_code == 201
    payload = response.json()
    assert payload["id"] != str(revision.id)
    assert payload["revision_hash"] != revision.revision_hash
    assert payload["source_item_ids"] == [str(source_item.id)]

    await session.refresh(revision)
    assert revision.status == BriefRevisionStatus.SUPERSEDED
